...
class MainController {
    ...
    public function main() {
        if (isset($_POST['subir_comision_colaborador']) && isset($_FILES['comisiones_file'])) {
            ...
            /*echo '<pre>';
            echo var_dump($no_listados);
            echo '</pre>';
            echo '######################################################';
            echo '<pre>';
            echo var_dump($ya_listada);
            echo '</pre>';*/

            $no_listados = is_array($no_listados) ? $no_listados : array_filter(explode("\n", trim($no_listados)));
            $ya_listada = is_array($ya_listada) ? $ya_listada : array_filter(explode("\n", trim($ya_listada)));
            ...
            if (count($no_listados) == 0 && count($ya_listada) == 0) {
                return $listados = array("reg_realizado" => '¡Registro realizado con éxito!');
            } else {
