...
def eliminar_carpetas_y_rar(directorio):
    # Revisa cada archivo y directorio en el directorio especificado
    for nombre in os.listdir(directorio):
        # Crea el camino completo al archivo o directorio
        camino = os.path.join(directorio, nombre)

        # Revisa si es un directorio y lo elimina
        if os.path.isdir(camino):
            shutil.rmtree(camino)
        # Revisa si es un archivo .rar y lo elimina
        elif os.path.isfile(camino) and nombre.endswith('.rar'):
            os.remove(camino)
            
directorio_especifico = '/'  # Reemplaza con el camino a tu directorio
